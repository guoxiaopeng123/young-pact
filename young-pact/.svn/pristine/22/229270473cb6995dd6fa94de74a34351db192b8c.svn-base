...
/**
 * @描述 : 托出合同驳回后超过24小时未重新提交审核定时器
 * @创建者 : guoxiaopeng
 * @创建时间 : 2018年9月7日 下午4:17:13
 */
@Service("rentBaseRejectTaskService")
public class RentBaseRejectTaskServiceImpl implements IScheduleTaskDealSingle<RentBaseVO>{
    private static final Log LOG = LogFactory.getLog(RentBaseRejectTaskServiceImpl.class);
    ...
    public List<RentBaseVO> selectTasks(String paramString1, String paramString2, int paramInt1, List<TaskItemDefine> paramList, int paramInt2) throws Exception {
        try {
            LOG.info("-----------托出合同驳回后超过24小时未重新提交审核定时任务   开始--------");
            List<RentBaseVO> rejectList = new ArrayList<>();
            ...
            if(null!=list&&list.size()>0){
                ...
                for(Map.Entry<String, List<OperateLogResult>> entry: groupOperateRecordMap.entrySet()){
                    ...
                    if(null !=operateLogList &&operateLogList.size()>0){
                        ...
                        String rentPactCode = entry.getKey();
                        boolean flag = checkApplyAuditLog(operateLogList);
                        if(!flag){
                            for(RentBaseVO rentBaseVO : list){
                                if( rentPactCode.equals(rentBaseVO.getRentPactCode())){
                                    rejectList.add(rentBaseVO);
                                    break;
                                }
                            }
                        }
                ...
                LOG.info("托出合同驳回后超过24小时未重新提交：" + JsonUtil.toJson(rejectList));
            }
            LOG.info("-----------托出合同驳回后超过24小时未重新提交审核定时任务  结束--------");
    ...
    public boolean execute(RentBaseVO rentBaseVO, String paramString) throws Exception {
        LOG.info("-----------托出合同驳回后超过24小时未重新提交审核定时任务执行结果   开始--------");
        ...
        if(flag){
            LOG.info("托出合同驳回后超过24小时未重新提交审核提醒----成功编码--"+rentBaseVO.getRentPactCode());
        }else{
            LOG.error("托出合同驳回后超过24小时未重新提交审核提醒----失败编码--"+rentBaseVO.getRentPactCode());
        }
        LOG.info("-----------托出合同驳回后超过24小时未重新提交审核定时任务执行结果  结束--------");
        return false;
    }
    /**
