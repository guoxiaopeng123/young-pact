...
/**
 * @描述 : 托进解约协议驳回后超过24小时未重新提交审核定时器
 * @创建者 : guoxiaopeng
 * @创建时间 : 2018年9月7日 下午1:53:28
 */
@Service("purminationRejectTaskService")
public class PurminationRejectTaskServiceImpl implements IScheduleTaskDealSingle<PurchaseTerminationVO>{
    private static final Log LOG = LogFactory.getLog(PurminationRejectTaskServiceImpl.class);
    ...
    public List<PurchaseTerminationVO> selectTasks(String paramString1, String paramString2, int paramInt1, List<TaskItemDefine> paramList, int paramInt2) throws Exception {
        try {
            LOG.info("-----------托进解约协议驳回后超过24小时未重新提交审核定时任务   开始--------");
            List<PurchaseTerminationVO> rejectList = new ArrayList<>();
            Map<String, List<OperateLogResult>> groupOperateRecordMap = Maps.newHashMap();
            List<String> serviceCodeList = null;
            PurchaseTerminationQuery purchaseBaseQuery = new PurchaseTerminationQuery();
            purchaseBaseQuery.setState(4);
            List<PurchaseTerminationVO> list = purchaseTerminationManager.listPurTerminationByParam(purchaseBaseQuery);
            if(null!=list&&list.size()>0){
                serviceCodeList = new ArrayList<>();
                for(PurchaseTerminationVO purchaseTerminationVO : list){
                    serviceCodeList.add(purchaseTerminationVO.getTerminationCode());
                }
                ...
                for(Map.Entry<String, List<OperateLogResult>> entry: groupOperateRecordMap.entrySet()){
                    ...
                    if(null !=operateLogList &&operateLogList.size()>0){
                        ...
                        String terminationCode = entry.getKey();
                        boolean flag = checkApplyAuditLog(operateLogList);
                        if(!flag){
                            for(PurchaseTerminationVO purchaseTerminationVO: list){
                                if( terminationCode.equals(purchaseTerminationVO.getTerminationCode())){
                                    rejectList.add(purchaseTerminationVO);
                                    break;
                                }
                            }
                        }
                ...
                LOG.info("托进解约协议驳回后超过24小时未重新提交：" + JsonUtil.toJson(rejectList));
            }
            LOG.info("-----------托进解约协议驳回后超过24小时未重新提交审核定时任务  结束--------");
    ...
    public boolean execute(PurchaseTerminationVO purchaseTerminationVO, String paramString) throws Exception {
        LOG.info("-----------托进解约协议驳回后超过24小时未重新提交审核定时任务执行结果   开始--------");
        ...
        if(flag){
            LOG.info("托进解约协议驳回后超过24小时未重新提交审核提醒----成功编码--"+purchaseTerminationVO.getTerminationCode());
        }else{
            LOG.error("托进解约协议驳回后超过24小时未重新提交审核提醒----失败编码--"+purchaseTerminationVO.getTerminationCode());
        }
        LOG.info("-----------托进解约协议驳回后超过24小时未重新提交审核定时任务执行结果  结束--------");
        return false;
    }
