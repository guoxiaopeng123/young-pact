...
/**
 * @描述 : 托出合同转租协议驳回后超过24小时未重新提交审核定时器
 * @创建者 : guoxiaopeng
 * @创建时间 : 2018年9月7日 下午6:20:34
 */
@Service("rentTurnRejectTaskService")
public class RentTurnRejectTaskServiceImpl implements IScheduleTaskDealSingle<RentTurnVO>{
    private static final Log LOG = LogFactory.getLog(RentTurnRejectTaskServiceImpl.class);
    ...
    public List<RentTurnVO> selectTasks(String paramString1, String paramString2, int paramInt1, List<TaskItemDefine> paramList, int paramInt2) throws Exception {
        try {
            LOG.info("-----------托出合同转租协议驳回后超过24小时未重新提交审核定时任务   开始--------");
            List<RentTurnVO> rejectList = new ArrayList<>();
            ...
            rentTurnQuery.setState(4);
            List<RentTurnVO> list = rentTurnManager.listRentTurnByParam(rentTurnQuery);
            if(null!=list&&list.size()>0){
                serviceCodeList = new ArrayList<>();
                for(RentTurnVO rentTurnVO : list){
                    serviceCodeList.add(rentTurnVO.getReletCode());
                }
                ...
                for(Map.Entry<String, List<OperateLogResult>> entry: groupOperateRecordMap.entrySet()){
                    ...
                    if(null !=operateLogList &&operateLogList.size()>0){
                        ...
                        String reletCode = entry.getKey();
                        boolean flag = checkApplyAuditLog(operateLogList);
                        if(!flag){
                            for(RentTurnVO rentTurnVO : list){
                                if(reletCode.equals(rentTurnVO.getReletCode())){
                                    rejectList.add(rentTurnVO);
                                    break;
                                }
                            }
                        }
                ...
                LOG.info("托出合同转租协议驳回后超过24小时未重新提交：" + JsonUtil.toJson(rejectList));
            }
            LOG.info("-----------托出合同转租协议驳回后超过24小时未重新提交审核定时任务  结束--------");
    ...
    public boolean execute(RentTurnVO rentTurnVO, String paramString) throws Exception {
        LOG.info("-----------托出合同转租协议驳回后超过24小时未重新提交审核定时任务执行结果   开始--------");
        ...
        if(flag){
            LOG.info("托出合同转租协议驳回后超过24小时未重新提交审核提醒----成功编码--" + rentTurnVO.getReletCode());
        }else{
            LOG.error("托出合同转租协议驳回后超过24小时未重新提交审核提醒----失败编码--" + rentTurnVO.getReletCode());
        }
        LOG.info("-----------托出合同转租协议驳回后超过24小时未重新提交审核定时任务执行结果  结束--------");
        return false;
    }
    /**
