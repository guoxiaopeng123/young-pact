...
/**
 * @描述 : 托出合同续租协议驳回后超过24小时未重新提交审核定时器
 * @创建者 : guoxiaopeng
 * @创建时间 : 2018年9月7日 下午6:20:34
 */
@Service("rentContuedRejectTaskService")
public class RentContuedRejectTaskServiceImpl implements IScheduleTaskDealSingle<RentContinuedVO>{
    private static final Log LOG = LogFactory.getLog(RentContuedRejectTaskServiceImpl.class);
    ...
    public List<RentContinuedVO> selectTasks(String paramString1, String paramString2, int paramInt1, List<TaskItemDefine> paramList, int paramInt2) throws Exception {
        try {
            LOG.info("-----------托出合同续租协议驳回后超过24小时未重新提交审核定时任务   开始--------");
            List<RentContinuedVO> rejectList = new ArrayList<>();
            ...
            rentContinuedQuery.setState(4);
            List<RentContinuedVO> list = rentContinuedManager.listRentcontinuedByParam(rentContinuedQuery);
            if(null!=list&&list.size()>0){
                serviceCodeList = new ArrayList<>();
                for(RentContinuedVO rentContinuedVO : list){
                    serviceCodeList.add(rentContinuedVO.getRenewCode());
                }
                ...
                for(Map.Entry<String, List<OperateLogResult>> entry: groupOperateRecordMap.entrySet()){
                    ...
                    if(null !=operateLogList &&operateLogList.size()>0){
                        ...
                        String renewCode = entry.getKey();
                        boolean flag = checkApplyAuditLog(operateLogList);
                        if(!flag){
                            for(RentContinuedVO rentContinuedVO : list){
                                if(renewCode.equals(rentContinuedVO.getRenewCode())){
                                    rejectList.add(rentContinuedVO);
                                    break;
                                }
                            }
                        }
                ...
                LOG.info("托出合同续租协议驳回后超过24小时未重新提交：" + JsonUtil.toJson(rejectList));
            }
            LOG.info("-----------托出合同续租协议驳回后超过24小时未重新提交审核定时任务  结束--------");
    ...
    public boolean execute(RentContinuedVO rentContinuedVO, String paramString) throws Exception {
        LOG.info("-----------托出合同续租协议驳回后超过24小时未重新提交审核定时任务执行结果   开始--------");
        ...
        if(flag){
            LOG.info("托出合同续租协议驳回后超过24小时未重新提交审核提醒----成功编码--" + rentContinuedVO.getRenewCode());
        }else{
            LOG.error("托出合同续租协议驳回后超过24小时未重新提交审核提醒----失败编码--" + rentContinuedVO.getRenewCode());
        }
        LOG.info("-----------托出合同续租协议驳回后超过24小时未重新提交审核定时任务执行结果  结束--------");
        return false;
    }
    /**
