...
/**
 * @描述 : 托出合同调房协议驳回后超过24小时未重新提交审核定时器
 * @创建者 : guoxiaopeng
 * @创建时间 : 2018年9月7日 下午6:20:34
 */
@Service("rentTransferRejectTaskService")
public class RentTransferRejectTaskServiceImpl implements IScheduleTaskDealSingle<PactRentTransferVO>{
    private static final Log LOG = LogFactory.getLog(RentTransferRejectTaskServiceImpl.class);
    ...
    public List<PactRentTransferVO> selectTasks(String paramString1, String paramString2, int paramInt1, List<TaskItemDefine> paramList, int paramInt2) throws Exception {
        try {
            LOG.info("-----------托出合同调房协议驳回后超过24小时未重新提交审核定时任务   开始--------");
            List<PactRentTransferVO> rejectList = new ArrayList<>();
            ...
            rentTransferQuery.setState(4);
            List<PactRentTransferVO> list = pactRentTransferManager.listTransferByParam(rentTransferQuery);
            if(null!=list&&list.size()>0){
                serviceCodeList = new ArrayList<>();
                for(PactRentTransferVO pactRentTransferVO : list){
                    serviceCodeList.add(pactRentTransferVO.getTransferCode());
                }
                ...
                for(Map.Entry<String, List<OperateLogResult>> entry: groupOperateRecordMap.entrySet()){
                    ...
                    if(null !=operateLogList &&operateLogList.size()>0){
                        ...
                        if(!flag){
                            for(PactRentTransferVO pactRentTransferVO : list){
                                if(transferCode.equals(pactRentTransferVO.getTransferCode())){
                                    rejectList.add(pactRentTransferVO);
                                    break;
                                }
                            }
                        }
                ...
                LOG.info("托出合同调房协议驳回后超过24小时未重新提交：" + JsonUtil.toJson(rejectList));
            }
            LOG.info("-----------托出合同调房协议驳回后超过24小时未重新提交审核定时任务  结束--------");
    ...
    public boolean execute(PactRentTransferVO pactRentTransferVO, String paramString) throws Exception {
        LOG.info("-----------托出合同调房协议驳回后超过24小时未重新提交审核定时任务执行结果   开始--------");
        ...
        if(flag){
            LOG.info("托出合同调房协议驳回后超过24小时未重新提交审核提醒----成功编码--" + pactRentTransferVO.getTransferCode());
        }else{
            LOG.error("托出合同调房协议驳回后超过24小时未重新提交审核提醒----失败编码--" + pactRentTransferVO.getTransferCode());
        }
        LOG.info("-----------托出合同调房协议驳回后超过24小时未重新提交审核定时任务执行结果  结束--------");
        return false;
    }
    /**
