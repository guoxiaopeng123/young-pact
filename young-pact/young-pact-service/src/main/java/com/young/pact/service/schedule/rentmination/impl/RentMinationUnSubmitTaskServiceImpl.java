...
/**
 * @描述 : 录入托出合同解约协议超过24小时未提交审核(合同草稿状态没有申请审核)
 * @创建者 : guoxiaopeng
 * @创建时间 : 2018年9月7日 下午6:21:42
 */
@Service("rentMinationUnSubmitTaskService")
public class RentMinationUnSubmitTaskServiceImpl implements IScheduleTaskDealSingle<PactRentTerminationVO>{

    private static final Log LOG = LogFactory.getLog(RentMinationUnSubmitTaskServiceImpl.class);
    ...
    public List<PactRentTerminationVO> selectTasks(String paramString1, String paramString2, int paramInt1, List<TaskItemDefine> paramList, int paramInt2) throws Exception {
        try {
            ...
            pactRentTerminationQuery.setState(1);
            List<PactRentTerminationVO> list= pactRentTerminationManager.listUnsubmitted(pactRentTerminationQuery);
    ...
    public boolean execute(PactRentTerminationVO pactRentTerminationVO, String paramString) throws Exception {
        LOG.info("-----------录入托出合同解约协议超过24小时未提交审核定时任务执行结果   开始--------");
        ...
        if(flag){
            LOG.info("录入托出合同解约协议超过24小时未提交审核提醒----成功编码--"+pactRentTerminationVO.getDissolutionCode());
        }else{
            LOG.error("录入托出合同解约协议超过24小时未提交审核提醒----失败编码--"+pactRentTerminationVO.getDissolutionCode());
        }
        LOG.info("-----------录入托出合同解约协议超过24小时未提交审核定时任务执行结果  结束--------");
        return true;
    }
